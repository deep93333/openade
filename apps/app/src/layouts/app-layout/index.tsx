    ...
    return (
      <TooltipProvider>
        <AgentServerOfflineOverlay
          ...
          manualConnectRequired={manualConnectRequired}
          onRetry={recheck}
        />
      </TooltipProvider>
    );
      ...
      <div className="flex h-screen w-full min-w-0 flex-col overflow-hidden bg-secondary dark:bg-background">
        ...
        <div className="light-theme-island flex min-h-0 min-w-0 gap-1 flex-1 flex-row overflow-hidden">
          <Group
            ...
            className="min-h-0 min-w-0 gap-1.5 px-1.5 pb-1.5 flex-1 gap-0"
          >
            <Panel id="main" minSize={360}>
              <div className={cn("flex min-w-0 h-full flex-col")}>
            ...
            {rightPanelOpen && (
              <div
                className={cn("flex h-full min-h-0 min-w-0 flex-row overflow-hidden", CARD_CLASS)}
              >
                <RightPanel onFileSelect={openFileViewer} showSeparator={!hasSecondaryPane} />
              </div>
            )}
          </Group>
        </div>
